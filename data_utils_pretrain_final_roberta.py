...
# bert_path = "D:/Projects/roberta-base-model"
...
class PreTrainDataset(Dataset):
    def __init__(self, data_set, tokenizer, emoji_tokenizer, sentiment_tokenizer):
        ...
        CLS_SEP_id = self.tokenizer.tokenizer.convert_tokens_to_ids(['<s>', "</s>"])
        # random_word = 0.2
        if data_set == "data_all":
            ...
            for name in name_list:
                path_save = "./data/processed/{}_roberta.pkl".format(name)
                if os.path.exists(path_save):
                    with open(path_save, 'rb') as fr:
                        data_all += pickle.load(fr)[: 1000000]
            self.data = data_all
            return
        path_save = "./data/processed/{}_roberta.pkl".format(data_set)
        if os.path.exists(path_save):
            with open(path_save, 'rb') as fr:
        ...
        for i in range(len(text_tokens)):
            ...
            if emoji_id != -1:
                ...
                if emoji_random_number < self.random_emoji:
                    text_token_masked.append(50264)
                else:
...
if __name__ == '__main__':
    ...
    # sentence = "this was a good story of the black leagues . i bought the book to teach in my high school reading class . i found it very informative and exciting . i would recommend to anyone interested in the history of the black leagues . it is well written , unlike a book of facts . the mckissack's continue to write good books for young audiences that can also be enjoyed by adults !"
    # tokens = tokenizer.tokenizer.tokenize(sentence)
    # print(tokens)
    # tokens_id = tokenizer.tokenizer.convert_tokens_to_ids(tokens)
    # print(tokens_id)
    #
    # tokens[0] = "<mask>"
    # tokens_id = tokenizer.tokenizer.convert_tokens_to_ids(tokens)
