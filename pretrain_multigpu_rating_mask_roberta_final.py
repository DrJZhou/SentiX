...
from data_utils_pretrain_final_roberta import Tokenizer4Bert, SentimentTokenizer, EmojiTokenizer, PreTrainDataset
from models.PretrainBERT_rating_mask_roberta_final import PretrainBERT
...
bert_path = "/home/zhoujie/roberta_model"
test_flag = False
...
class Instructor:
    ...
    def _train(self, train_data_loader, val_data_loader):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(train_data_loader):
                ...
                if global_step % int(batch_num/40) == 0:
                    ...
                    if not os.path.exists('state_dict'):
                        os.mkdir('state_dict')
                    path = 'state_dict/{0}_{1}_val_acc_{2}_{3}_Rating_Mask_roberta'.format(self.opt.model_name,
                                                                                   self.opt.dataset,
                                                                                   round(val_acc[0], 4),
                                                                                   round(val_acc[1], 4))
                    try:
                    ...
                    path_bert = 'state_dict/roberta_{0}_{1}_val_acc_{2}_{3}_Rating_Mask'.format(self.opt.model_name,
                                                                                             self.opt.dataset,
                                                                                             round(val_acc[0], 4),
                                                                                             round(val_acc[1], 4))
    ...
    def _evaluate_acc_f1(self, data_loader):
        n_correct, n_total, n_total_word = np.array([0.0, 0.0]), 0, 0
...
def main():
    ...
    parser.add_argument('--learning_rate', default=6e-4, type=float, help='try 5e-5, 2e-5 for BERT, 1e-3 for others')
