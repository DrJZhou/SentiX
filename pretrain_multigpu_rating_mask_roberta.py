...
from models.PretrainBERT_rating_mask_roberta import PretrainBERT
            ...
            for i_batch, sample_batched in enumerate(train_data_loader):
                ...
                if global_step % int(batch_num/40) == 0:
                    ...
                    if not os.path.exists('state_dict'):
                        os.mkdir('state_dict')
                    path = 'state_dict/Pretrain_roberta_{0}_{1}_val_f1_{2}_meanf1_{3}_Rating_Mask_{4}_epoch_{5}'.format(self.opt.model_name,
                                                                                         self.opt.dataset,
                                                                                         round(val_f1[0], 4),
                    ...
                    path_bert = 'state_dict/roberta_{0}_{1}_val_f1_{2}_meanf1_{3}_Rating_Mask_{4}_epoch_{5}'.format(self.opt.model_name,
                                                                                                   self.opt.dataset,
                                                                                                   round(val_f1[0], 4),
                                                                                                   round(np.mean(val_f1),4), i_batch, epoch)
