...
from data_utils_pretrain_rating import Tokenizer4Bert, SentimentTokenizer, EmojiTokenizer, PreTrainDataset
from models.PretrainBERT_rating import PretrainBERT
...
class Instructor:
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            n_correct, n_total_loss, loss_total = 0, 0, 0
            ...
            for i_batch, sample_batched in enumerate(train_data_loader):
                ...
                n_correct += (torch.argmax(outputs, -1) == rating).sum().item()
                loss_total += loss.item() * len(inputs[0])
                n_total_loss += len(inputs[0])
                if global_step % self.opt.log_step == 0:
                    train_loss = loss_total / n_total_loss
                    train_acc_rating = n_correct / n_total_loss
                    logger.info(
                        '{}/{}---loss: {:.4f}, acc: {:.4f}'.format(i_batch, batch_num, train_loss, train_acc_rating))

                if i_batch % int(batch_num/80) == 0 and i_batch != 0:
                    ...
                    if val_acc > max_val_acc:
                        ...
                        if not os.path.exists('state_dict'):
                            os.mkdir('state_dict')
                        path = 'state_dict/{0}_{1}_val_f1_{2}_acc_{3}_Rating'.format(self.opt.model_name, self.opt.dataset,
                                                                     round(val_f1, 4), round(val_acc, 4))
                        try:
                            torch.save(self.model.state_dict(), path)
                        except:
                            torch.save(self.model.module.state_dict(), path)
                        path_bert = 'state_dict/bert_{0}_{1}_val_f1_{2}_acc_{3}_Rating'.format(self.opt.model_name, self.opt.dataset,
                                                                               round(val_f1, 4), round(val_acc, 4))
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(data_loader):
                ...
                n_correct += (torch.argmax(t_outputs, -1) == t_rating).sum().item()
                t_outputs = t_outputs.cpu()
                t_rating = t_rating.cpu()
                # if t_batch < 1:
                #     print(t_outputs, t_rating)
                #     print(t_inputs[0])
                n_total += len(t_inputs[0])
                if t_targets_all is None:
                    t_targets_all = t_rating
                    t_outputs_all = t_outputs
                else:
                    t_targets_all = torch.cat((t_targets_all, t_rating), dim=0)
                    t_outputs_all = torch.cat((t_outputs_all, t_outputs), dim=0)

                if t_batch % 5 == 0:
                    # print(t_targets_all, t_outputs_all)
                    n_f1 += metrics.f1_score(t_targets_all, torch.argmax(t_outputs_all, -1), labels=[0, 1, 2, 3, 4], average='macro')
                    t_targets_all, t_outputs_all = None, None
                    n_total_f1 += 1
