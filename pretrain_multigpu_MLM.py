...
from data_utils_pretrain_MLM_final import Tokenizer4Bert, SentimentTokenizer, EmojiTokenizer, PreTrainDataset
from models.PretrainBERT_MLM_final import PretrainBERT
            ...
            for i_batch, sample_batched in enumerate(train_data_loader):
                ...
                if global_step % int(batch_num/20) == 0:
                    ...
                    if not os.path.exists('state_dict'):
                        os.mkdir('state_dict')
                    path = 'state_dict/{0}_{1}_val_acc_{2}_MLM'.format(self.opt.model_name,
                                                                                         self.opt.dataset,
                                                                                         round(val_acc[0], 4))
                    try:
                    ...
                    path_bert = 'state_dict/bert_{0}_{1}_val_acc_{2}_MLM'.format(self.opt.model_name,
                                                                                                   self.opt.dataset,
                                                                                                   round(val_acc[0], 4))
