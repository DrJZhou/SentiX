...
class Instructor:
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            if val_acc > max_val_acc:
                ...
                if not os.path.exists('state_dict'):
                    os.mkdir('state_dict')
                path = 'state_dict/few_shot_{0}_{1}_val_temp'.format(self.opt.model_name, self.opt.dataset)
    ...
    def run(self):
        ...
        test_data_loader = DataLoader(dataset=self.testset, batch_size=32, shuffle=False)
        if self.opt.cross_val_fold > 0:
            ...
            for fid in range(self.opt.cross_val_fold):
                ...
                val_data_loader = DataLoader(dataset=valset, batch_size=32, shuffle=False)
...
def main():
    ...
    parser.add_argument('--few_shot_num', default=100, type=int, help='few shot num')
    parser.add_argument(
        "--fp16",
        action="store_true",
    ...
    log_file = 'results/few_shot_sentiment_analysis/few_shot_{}-{}-{}.log'.format(opt.model_name, opt.dataset, strftime("%y%m%d-%H%M", localtime()))
