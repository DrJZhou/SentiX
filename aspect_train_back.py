...
from data_utils_aspect import Tokenizer4Bert, ABSADataset

from models.AspectLevelSentimentClassification import AspectLevelSentimentClassification
...
class Instructor:
    def __init__(self, opt):
        ...
        if not self.opt.pretrained_bert_name:
            self.opt.pretrained_bert_name = bert_path
        else:
            self.opt.pretrained_bert_name = self.opt.pretrained_bert_name.replace("\r", "").replace("\n", "")
        bert = BertModel.from_pretrained(self.opt.pretrained_bert_name)
        self.pretrained_bert_state_dict = bert.state_dict()
        self.model = opt.model_class(bert, opt).to(opt.device)

        self.trainset = ABSADataset(opt.dataset, tokenizer, train_flag='train')
        self.testset = ABSADataset(opt.dataset, tokenizer, train_flag='test')
...
def main():
    ...
    parser.add_argument('--model_name', default='aspect-based_sentiment_analysis', type=str)
    parser.add_argument('--dataset', default='twitter', type=str, help='twitter, res14, lap14')
    ...
    parser.add_argument('--pretrained_bert_name', default=None, type=str)
    parser.add_argument('--max_seq_len', default=80, type=int)
    parser.add_argument('--sentiment_class', default=3, type=int)
    ...
    model_classes = {
        'aspect-based_sentiment_analysis': AspectLevelSentimentClassification,
    }

    input_colses = {
        'aspect-based_sentiment_analysis': ['text_bert_indices', 'bert_segments_ids'],
    }
    initializers = {
    ...
    log_file = 'results/aspect_based_sentiment_analysis/{}-{}-{}.log'.format(opt.model_name, opt.dataset, strftime("%y%m%d-%H%M", localtime()))
